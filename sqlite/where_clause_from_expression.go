...
func WhereClauseFromExpression(expr expressions.Expression, idToSQLiteIdent map[expressions.ID]string) (string, []any, error) {
	if expr == nil {
		return "", nil, nil
	}
	switch expr := expr.(type) {
	case *expressions.And:
		left, leftBinds, err := WhereClauseFromExpression(expr.Left(), idToSQLiteIdent)
		if err != nil {
			return "", nil, err
		}
		right, rightBinds, err := WhereClauseFromExpression(expr.Right(), idToSQLiteIdent)
		if err != nil {
			return "", nil, err
		}
	...
	case *expressions.Not:
		operand, binds, err := WhereClauseFromExpression(expr.Operand(), idToSQLiteIdent)
		if err != nil {
			return "", nil, err
		}
